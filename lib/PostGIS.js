...
module.exports = {
  ...
  oneFilter: function( clause ){
    return ' feature->\'properties\'->>\''+clause.left.replace(/'/g,'')+'\' ' + clause.operator + '\'' + clause.right.replace(/'/g,'') + '\''; 
  },
...
=======
            //console.log(select);
>>>>>>> better sql parsing logic and tests
          }
  ...
  _insertFeature: function(table, feature, i){
    ...
    var sql = 'insert into "'+table+'" (feature, geom) VALUES (\''+JSON.stringify(feature).replace(/'/g, "")+'\', ST_GeographyFromText(ST_AsText(ST_GeomFromGeoJSON(\''+JSON.stringify(feature.geometry)+'\'))))' ;
    this._query(sql);
  },
