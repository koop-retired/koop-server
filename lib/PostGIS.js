    ...
    this._query('select info from "'+this.infoTable+'" where id=\''+(key+':'+(options.layer || 0 )+":info")+'\'', function(err, result){
      if ( err || !result || !result.rows || !result.rows.length ){
        ...
        callback( null, [{ status: 'processing' }]);
          ...
          var select;
          if (options.simplify){
            var t = Math.pow(1 / options.zoom, 2);
            select = 'select feature->>\'id\' as id, feature->>\'properties\' as props, st_asgeojson(st_simplifypreservetopology(geom::geometry, '+t+')) as geom from "' + key+':'+(options.layer || 0)+'"'; 
          } else {
            select = 'select feature->>\'id\' as id, feature->>\'properties\' as props, feature->>\'geometry\' as geom from "' + key+':'+(options.layer || 0)+'"'; 
          }
          ...
          if ( options.where ) { 
            if ( options.where != '1=1'){
              ...
              var where = sql.sql2ast('select * from foo where '+ options.where).WHERE;
              console.log(where);
              var clause = '';
          ...
          self._query( select.replace(/ feature->>'id' as id, feature->>'properties' as props, feature->>'geometry' as geom /, ' count(*) as count '), function(err, result){ 
