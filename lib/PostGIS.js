...
module.exports = {
  ...
  _insertFeature: function(table, feature, i){
    ...
    //var sql = 'insert into "'+table+'" (feature, geom) VALUES (\''+JSON.stringify(feature).replace(/'/g, "")+'\', ST_AsText(ST_GeomFromGeoJSON(\''+JSON.stringify(feature.geometry)+'\')))' ;
    this._query(sql);
  },
  ...
  _query: function(sql, callback){
    ...
    pg.connect(config.db.postgis.conn, function(err, client, done) {
      if(err) {
