...
module.exports = {
  ...
  getCount: function( key, options, callback ){
    ...
    this._query(select, function(err, result){
      if ( err || !result || !result.rows || !result.rows.length ){
        ...
        console.log('Get Count', result.rows[0].count, select);
  ...
  parseGeometry: function( geometry ){
    ...
    if ( typeof( geom ) == 'string' ){
      try {
        ...
        //console.log('Error parsing options.geometry, splitting string', geom);
        try {
          ...
          console.log('Error building bbox from', geometry);
        }
      }
    }
    return geom;
  ...
  insertPartial: function( key, geojson, layerId, callback ){
    ...
    this._query(sql, function(err, res){
      console.log('insert partial', err);
      callback(null, true);
    });
  },
  ...
  _buildSchemaFromFeature: function(feature){
    var schema = '(';
    var props = ['id SERIAL PRIMARY KEY', 'feature JSON', 'geom Geometry('+feature.geometry.type.toUpperCase()+', 4326)'];
    schema += props.join(',') + ')';
    return schema;
  }
