...
exports.taskQueue = async.queue(function (task, callback) {
  ...
  Cache.db.select( task.key, task.options, function( err, result ){
    finish(task.dir, task.key +":"+ (task.options.layer || 0), task.options, result[0], task.hash, callback);
  }); 
  ...
  Cache.getInfo(key, function(err, info){
    ...
    if (info.format){
      ...
      Cache.get( keys[0], keys[1], options, function(err, entry ){
        if (!err){
          Exporter.exportToFormat( info.format, dir, hash, entry[0], function(err, result){
            delete info.format; 
            _update( info );
          }); 
        }
