...
var Exporter = function( koop ){
  ...
  this.exportLarge = function( format, id, key, type, options, done ){
                  ...
                  mv(shpdir+'/OGRGeoJSON.dbf', base+'/' + options.name + '.dbf', function(err) {
                    mv(shpdir+'/OGRGeoJSON.shx', base+'/' + options.name + '.shx', function(err) {
                      mv(shpdir+'/OGRGeoJSON.prj', base+'/' + options.name + '.prj', function(err) {
                        ...
                        child = exec(['zip', '-rj', newZip, base+'/', '-x', base + '/*.json'].join(' '), function (err, stdout, stderr) {
    ...
    var path = [koop.Cache.data_dir + 'files', dir].join('/');
    var base = path + '/' + key,
      vrtFile = base + '.vrt',
      jsonFile = base + '.json',
      newFile = base + '.' + format;
    ...
    if ( fs.existsSync(jsonFile) && (format == 'json' || format == 'geojson') ){
      ...
      koop.Cache.getInfo(table, function(err, info){
        ...
        _update( info, function(err, res){
          ...
          fs.mkdir( base, '0777', true, function(){ 
            koop.Cache.getCount(table, function(err, count){
  ...
  this.exportToFormat = function( format, dir, key, geojson, options, callback ){
    ...
    var _callOgr = function(inFile, outFile, callback){
      if (format == 'json' || format == 'geojson'){
        ...
        var cmd = ['ogr2ogr', '--config','SHAPE_ENCODING','UTF-8', '-f', self.ogrFormats[format], ( format == 'zip' ) ? outFile.replace('zip','shp') : outFile, inFile];
        if (format == 'csv') {
                ...
                if ( format == 'zip' ){
                  ...
                  fs.mkdir( base, '0777', true, function(){
                    ...
                    if ( options.name ){
                      ...
                      fs.writeFileSync(base+'/' + options.name + '.cpg', fs.readFileSync(cpg));

                      // zip all and return the new zip
                      var newZip = base + '/' + options.name + '.zip';
                      exec(['zip', '-rj', newZip, base+'/'].join(' '), function(err, stdout, stderr){
                        callback(null, newZip);
                      });
                    } else {
    ...
    var _send = function(err, file){
      if (err){
        ...
        // TODO revisit the centralized disk access 
    ...
    koop.Cache.getInfo(key, function(err, info){
      if (info) {
        ...
        if (info.format){
          ...
          koop.Cache.getCount(key, function(err, count){
            if ( count > limit ){
