  ...
  exportLarge: function( format, id, key, type, options, done ){
        ...
        if ( format == 'zip' || format == 'shp'){
          ...
          fs.mkdir( base, '0777', true, function(){
            ...
            if ( options.name ){
              ...
              mv(shp, base+'/' + options.name + '.shp', shp, function(err) {
                mv(dbf, base+'/' + options.name + '.dbf', dbf, function(err) {
                  mv(shx, base+'/' + options.name + '.shx', shx, function(err) {
                    mv(prj, base+'/' + options.name + '.prj', prj, function(err) {
      ...
      fs.writeFile(file, JSON.stringify(json), function(){
        ...
        if (fileCount == pages){
          ...
          fs.writeFile(vrtFile, vrt, function(){
            // CALL OGR
            console.log('calling ogr2ogr - VRT to json', fs.existsSync(vrtFile) );
            var cmd = ['ogr2ogr', '-f', 'GeoJSON', '-append', jsonFile, vrtFile];
            callOGR('vrt', jsonFile, cmd, function(err, vrtOutFile){
              if (format == 'json' || format == 'geojson'){
                done(null, jsonFile.replace('geojson', 'json'));
              } else {
                //convert to format 
                cmd = ['ogr2ogr', '-f', self.ogrFormats[format], ( format == 'zip' ) ? newFile.replace('zip','shp') : newFile, vrtOutFile];
    ...
    var q = async.queue(function (task, cb) {
      var opts = {
        ...
        where: options.where,
        offset: task.offset,
        bypassProcessing: options.bypassProcessing
      };
    ...
    if ( fs.existsSync(jsonFile) && (format == 'json' || format == 'geojson') ){
      ...
      Cache.getInfo(table, function(err, info){
        ...
        _update( info, function(err, res){ 
          ...
          cmd = ['ogr2ogr', '-f', self.ogrFormats[format], ( format == 'zip' ) ? newFile.replace('zip','shp') : newFile, jsonFile];
          callOGR(format, newFile, cmd, function(err, formatFile){
             delete info.status;
