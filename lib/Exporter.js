...
exports.exportToFormat = function( format, dir, key, geojson, callback ){
    ...
    var _callOgr = function(inFile, outFile, callback){
      if (format == 'json'){
        ...
        config.worker.aspawn(['ogr2ogr', '-f', ogrFormats[format], ( format == 'zip' ) ? outFile.replace('zip','shp') : outFile, inFile],
      ...
      if (err){
        ...
        if (config.peechee && config.peechee.type == 's3'){ 
          fs.readFile(file, function (err, data) {
            ...
            config.peechee.write(data, dir, key+'.'+format, function(err,res){
              callback( null, file );
            })
          });
        } else {
