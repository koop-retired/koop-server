...
var Exporter = function( koop ){
  ...
  if ( koop.config.export_workers ){
    this.export_q = kue.createQueue({
      prefix: 'export-q',
      redis: {
        port: koop.config.export_workers.port,
        host: koop.config.export_workers.host
      }
    });
  }
    ...
    if (fs.existsSync(vrtFile) && !options.ignore_cache) {
      ...
      koop.Cache.getInfo(table, function(err, info){
        ...
        _update( info, function(err, res){ 
          ...
          console.log('calling ogr', format);
          ...
          fs.mkdir( base, '0777', true, function(){ 
            koop.Cache.getCount(table, options, function(err, count){
                  ...
                  for (var i = 0; i < pages; i++){
                    (function(){
                      var j = i;
                      var offset = j * (options.limit);
                      var op = { file: base+'/part.' + j + '.json', offset: offset }; 
                      if ( koop.config.export_workers ){
                        op.options = options;
                        op.options.key = key;
                        op.dbkey = dbkey;
                        ...
                        info.export_workers = {total: pages, complete: 0, failed: 0, vrt:'<OGRVRTDataSource>'};
                        _update( info, function(err, res){
                          var job = self.export_q.create( 'exports', op).attempts( 3 ).save( function( err ){
  ...
  this.exportToFormat = function( format, dir, key, geojson, options, callback ){
    ...
    var jsonFile = (options.name || key) + '.json',
      newFileTmp = key + tmpName + '.' + format,
      newFile = (options.name || key) + '.' + format;
