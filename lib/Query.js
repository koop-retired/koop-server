        ...
        if ( params.groupByFieldsForStatistics ){
          this.buildUniqueFeatures(params, json.features, function( err, uniques ){
            ...
            for ( var value in uniques ){
              console.log(uniques[ value ]);
              var statResult = this.buildStats( stats, { features: uniques[ value ] }, params );
              statCollection.push( statResult );      
            }
       
            console.log(statCollection[0])
            //self.buildStats(stats, json, params, callback);
          });
        } else {
