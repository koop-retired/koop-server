...
exports.insert = function( type, key, data, layerId, callback ){
  ...
  this.db.insert( type+':'+key, data, layerId, function(err, success){
    self.db.timer.set( type+':'+key+':timer', 3600000, function( error, timer){
...
exports.process = function( type, key, data, options, callback ){
  ...
  if ( !data.length ){
    ...
    this.db.timer.get( timerKey, function( error, timer){
      if ( timer ){
        ...
        if ( global[type] && global[type].checkthis ) {
          global[type].checkthis( key, data, options, function(err, success){
            if ( !success ){
              ...
              self.db.timer.set( timerKey, checkTime, function( error, timer){
                callback( null, data );
              });
            } else {
...
// removes everything Koop knows about a dataset 
// will purge the DB cache and the filesystem 
exports.removeAll = function(type, key, options, callback){
  var self = this;
