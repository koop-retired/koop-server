...
exports.remove = function(type, key, options, callback){
  ...
  Cache.db.remove( type+':'+key+':'+(options.layer || 0), function(err, result){
    // Remove all files
    self.rmDir( [ config.data_dir+'files/'+type, key, (options.layer || 0) ].join(':'), function(err, res){
      self.rmDir( [ config.data_dir+'tiles/'+type, key, (options.layer || 0) ].join(':'), function(err, res){
        self.rmDir( [ config.data_dir+'thumbs/'+type, key, (options.layer || 0) ].join(':'), function(err, res){
          if (config.peechee){
            console.log('must rm bucket subdir on s3');
          }
          if ( callback ) { 
            callback(null, true);
          }
