...
module.exports = {
  ...
  insert: function( key, geojson, layerId, callback ){
    console.log(key, geojson.features.length);
    var self = this;
    var info = {},
      count = 0;
      error = null;
  ...
  services: {
    register: function( type, info, callback){
      SQLite._createTable(type, '( id varchar(100), host varchar(100))', function(err){
        SQLite._query('select * from "'+type+'" where id=\''+info.id+"\'", function(err, rows){
          if ( err || !rows || !rows.length ) {
            var sql = 'insert into "'+type+'" (id, host) VALUES (\''+info.id+'\', \''+info.host+'\')' ;
            SQLite._run(sql, function(err){
              callback( err, true );
            });
          } else {
            callback( err, true );
    ...
    count: function( type, callback){
      ...
      SQLite._query(sql, function(err, rows){
        if (err || !rows || !rows.length){
          callback( err, 0 );
        } else {
  ...
  timer: {
    set: function(key, expires, callback){
      ...
      SQLite._run('delete from "'+ SQLite.timerTable +'" WHERE id=\''+key+"\'", function(err){
        SQLite._run('insert into "'+ SQLite.timerTable +'" (id, expires) VALUES (\''+key+'\', \''+expires_at.getTime()+'\')', function(err){
          callback( err, null );
        });
      });
    },
    get: function(key, callback){
      SQLite._query('select * from "'+ SQLite.timerTable + '" where id=\''+key+"\'", function(err, rows){
        if (err || !rows || !rows.length){
          callback( err, null);
        } else {
